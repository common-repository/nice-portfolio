...
/**
 * Class Nice_Portfolio_MetaboxCreateResponder
 *
 * This class takes charge of the interaction of created Nice_Portfolio_Metabox
 * instances with WordPress APIs.
 *
 * @since 1.1
 */
class Nice_Portfolio_MetaboxCreateResponder extends Nice_Portfolio_CreateResponder {
	/**
	...
	protected function get_metabox_settings( $post_type ) {
		...
		$defaults = array(
			'id' 			=> $data->id ? : 'nice-portfolio-' . $post_type . '-settings',
			'title' 		=> sprintf( esc_html__( '%s Settings', 'nice-portfolio' ), ucfirst( $post_type ) ),
			'callback' 		=> 'nice_portfolio_metabox_print',
			'post_type'     => $post_type,
		...
		$settings = apply_filters( 'nice_portfolio_metabox_' . $post_type . '_settings', $settings );

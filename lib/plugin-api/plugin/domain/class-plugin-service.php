...
/**
 * Class Nice_Portfolio_PluginService
 *
 * This class deals with typical operations over Nice_Portfolio_Plugin instances.
 *
 * @package Nice_Portfolio_Plugin_API
 * @author  NiceThemes <hello@nicethemes.com>
 * @since   1.0
 */
class Nice_Portfolio_PluginService extends Nice_Portfolio_Service {
	/**
	...
	protected static function get_init_data() {
		...
		$data['plugin_file']            = nice_portfolio_plugin_file( true );
		$data['plugin_domain']          = nice_portfolio_plugin_domain( true );
		...
		$data['using_nicetheme']        = nice_portfolio_is_nicetheme( $data['theme_template'] );
		$data['settings_name']          = 'nice_portfolio_settings';
	 ...
	 * @since 1.0
	 *
	 * @param Nice_Portfolio_Plugin|null $plugin
	 */
	public static function refresh_settings( $plugin = null ) {
		if ( ! $plugin instanceof Nice_Portfolio_Plugin ) {
		 ...
		 */
		nice_portfolio_update( $plugin, array(
	 ...
	 */
	public static function set_supported_integrations( Nice_Portfolio_Plugin $plugin ) {
		/**
		 * Update plugin instance.
		 */
		nice_portfolio_update( $plugin, array(
	 ...
	 *
	 * @uses  nice_portfolio_integration_create()
	 *
	 * @since 1.0
	 *
	 * @return array
	...
	protected static function get_supported_integrations() {
		/**
		 * @hook nice_portfolio_supported_integrations
		 *
		 ...
		 * @since 1.0
		 */
		$integrations = apply_filters( 'nice_portfolio_supported_integrations', array() );
		...
		if ( ! empty( $integrations ) ) {
			foreach ( $integrations as $data ) {
				/**
				 * Create instances of `Nice_Portfolio_Integration` with the
				 * received data and add them to our collection.
				 *
				 * @see Nice_Portfolio_Integration
				 */
				$supported_integrations[] = nice_portfolio_integration_create( $data );
			}
		}
	 ...
	 * @param Nice_Portfolio_Plugin $plugin
	 */
	public static function set_i18n( Nice_Portfolio_Plugin $plugin ) {
		/**
		 * Update plugin instance.
		 */
		nice_portfolio_update( $plugin, array(
	...
	protected static function get_i18n() {
		 ...
		 */
		return nice_portfolio_i18n_create( apply_filters( 'nice_portfolio_plugin_i18n_data', array(
	...
	protected static function get_post_types() {
		$post_types_data = apply_filters( 'nice_portfolio_post_types', array() );
		$post_types      = array();

		if ( ! empty( $post_types_data ) ) {
			foreach ( $post_types_data as $data ) {
				$post_types[] = nice_portfolio_post_type_create( $data );
			}
		}
